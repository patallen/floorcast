...
class WebsocketService:
    ...
    async def _handle_entity_state_change_event(self, event: EntityStateChanged) -> None:
        for client in self._clients:
            if "entity_states" in client.subscriptions:
    ...
    @staticmethod
    def _handle_subscribe(conn: WSConnection, subscription: str) -> None:
        if subscription not in ("entity_states",):
            raise ValueError(f"Unknown subscription: {subscription}")
        conn.subscribe(subscription)

    @staticmethod
    def _handle_unsubscribe(conn: WSConnection, subscription: str) -> None:
        if subscription not in ("entity_states",):
            raise ValueError(f"Unknown subscription: {subscription}")
